...
//init table
//db.CreateTable(&mode1,&mode2,&model3).Error  可以同时创建多个数据库表
...
//根据ID查找Product信息
//多表查询db.Preload(OUTKEY1,OUTKEY2,OUTKEY3).First(out mode struct,condition)
...
//创建Product信息
//创建记录 db.Crate(mode.struct).Error
...
func (u *ProductRepository) DeleteProductByID(productID int64) error {
	...
	//you can use 'Unscoped' delete match record
